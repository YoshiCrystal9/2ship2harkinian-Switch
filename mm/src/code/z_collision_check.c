...
s32 CollisionCheck_NoSharedFlags(ColliderElement* atElem, ColliderElement* acElem) {
    if (GameInteractor_Should(VB_CHECK_BUMPER_COLLISION, !(atElem->toucher.dmgFlags & acElem->bumper.dmgFlags),
                              atElem, acElem)) {
        return true;
    }
    return false;
...
void CollisionCheck_AC_SphereVsQuad(struct PlayState* play, CollisionCheckContext* colCtxt, Collider* colAT,
    ...
    if (Math3D_TriVsSphIntersect(&at->dim.worldSphere, &D_801EE6C8.norm1, &hitPos) ||
        Math3D_TriVsSphIntersect(&at->dim.worldSphere, &D_801EE6C8.norm2, &hitPos)) {
        Vec3f atPos;
        Vec3f acPos;
